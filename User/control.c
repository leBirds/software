...
u8 DrYL_PID_Control_pitch_roll(void)             
{
         ...
         if(TIM2_IRQCNT>10)
        {
            ...
            DrYL_Set_PID(10,0,0,&pitch_pid);//设置pitch  pid参数
            DrYL_Set_PID(10,0,0,&roll_pid);//设置roll  pid 参数
            pitch_pid_result=DrYL_IncPID_Calc(Yaw,&pitch_pid); // 计算增量型参数
            roll_pid_result=DrYL_IncPID_Calc(Roll,&roll_pid); //
            // 一个调节周期之内不会让它一直上升,只调节一对电机，另一对电机作为参考 
            if((pitch_pid.LastError<0.1)&&(pitch_pid.LastError>-0.1))
            {
            ...
            if((roll_pid.LastError<0.1)&&(roll_pid.LastError>-0.1))
            {
