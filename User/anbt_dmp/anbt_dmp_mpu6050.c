...
u8 AnBT_DMP_MPU6050_DEV_CFG(void)		
{
	  unsigned char anbt_dmp_data[6], anbt_dmp_rev;
		//
	  if (AnBT_DMP_I2C_Read(st.hw->addr, st.reg->who_am_i, 1, &(anbt_dmp_data[0]))) return 1;   
	  if (anbt_dmp_data[0]!=PRODUCT_WHOAMI) 
	  {
               // AnBT_Uart1_Send_String("WHOAMI error",12); 
               // AnBT_Uart1_Send_Num(anbt_dmp_data[0]);
                return 2;    
          }
		//
          anbt_dmp_data[0] = 0x80;	//etootle: BIT_RESET, Reset device
          if (AnBT_DMP_I2C_Write(st.hw->addr, st.reg->pwr_mgmt_1, 1, &(anbt_dmp_data[0]))) return 3;   
          AnBT_DMP_Delay_ms(100);
                  //
          anbt_dmp_data[0] = 0x00; //etootle: Wake up chip
          if (AnBT_DMP_I2C_Write(st.hw->addr, st.reg->pwr_mgmt_1, 1, &(anbt_dmp_data[0]))) return 4;
    //
          if (AnBT_DMP_I2C_Read(st.hw->addr, st.reg->accel_offs, 6, anbt_dmp_data))	return 5;	//etootle:Check product revision  
          ...
          if (anbt_dmp_rev)  //Congrats, these parts are better
	  {
          ...
          else 
          {
                  ...
                  if (!anbt_dmp_rev) 
                  {
                       // AnBT_Uart1_Send_String("Product ID error",16); 
                        return 5;    
                  } 
...
void AnBT_DMP_MPU6050_SEND_DATA_FUN(void)	
{
	...
	//ANBT_SEND_DMP_DATA(anbt_mpu6050_quat_data);
}
